...
router.post("/login", async (req, res) => {
  try {
    ...
    const findUser = await user.findOne({$or:[{emailId},{mobilenumber:isNumber}]});
    // const findUser = await user.findOne({mobilenumber:emailId,password});
    if (!findUser) {
      ...
      bcrypt.compare(password,findUser.password).then((result)=>{
        ...
        res.send({ message: `Successfully logged in`, name:findUser.username,token });
      }
      else
...
router.post("/register", async (req, res) => {
  try {
    ...
    if (!checkUser) {
      ...
      res.send({ message: `${username} registered successfully`, name:username,token });
    } else {
